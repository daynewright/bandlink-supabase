-- SOURCE: https://mansueli.hashnode.dev/streamlining-postgresql-function-management-with-supabase
...
-- Managing functions often involves deploying them from source code. 
-- PostgreSQL requires specific syntax for function creation, and Supabase simplifies this with the create_function_from_source function.
...
-- ROLLING BACK FUNCTIONS
-- The rollback_function function comes to the rescue. It retrieves the most recent function version from the archive.function_history table and executes it. 
-- If no previous version exists, it gracefully handles the situation.
...
-- SETUP ALL CURRENT FUNCTIONS IN HISTORY
